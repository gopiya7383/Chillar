...
FIREBASE_URL = os.getenv("FIREBASE_URL", "")  # Optional - can be set via bot
...
class ReferralBot:
    ...
    def _get_keyboard(self, user_id: int) -> InlineKeyboardMarkup:
        ...
        keyboard.append([InlineKeyboardButton("🔑 Set Firebase URL", callback_data="set_firebase")])
    ...
    async def button_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == "set_firebase":
            await query.edit_message_text(
                f"🔑 **Set Firebase URL**\n\n"
                f"Please send your Firebase Realtime Database URL.\n\n"
                f"Format: `https://stockgro-xxxx.firebaseio.com/`\n\n"
                f"Type or paste the URL below:",
                parse_mode='Markdown'
            )
            return WAITING_FOR_FIREBASE_URL
        ...
        if not firebase_url:
            await query.edit_message_text(
                ...
                "Please click the 'Set Firebase URL' button first.",
                parse_mode='Markdown',
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔑 Set Firebase URL", callback_data="set_firebase")],
    ...
    async def show_progress(self, query, user_id: int):
        if not self.monitor:
            await query.edit_message_text("⚠️ Monitor not initialized. Please set Firebase URL first.")
            return
    ...
    async def handle_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Check if waiting for Firebase URL
        if context.user_data.get('waiting_for_firebase'):
            return await self.handle_firebase_url_input(update, context)
    ...
    def run(self):
        ...
        conv_handler = ConversationHandler(
            entry_points=[
                ...
                CallbackQueryHandler(self.button_handler, pattern="set_firebase")
            ],
            states={
