...
Python 3.11 Compatible - Polling Mode
"""
import os
import sys
import json
...
class FirebaseMonitor:
    def __init__(self, firebase_url: str):
        ...
        logger.info("FirebaseMonitor initialized")
    ...
    def _fetch_initial_state(self):
        try:
            ...
            if resp.status_code == 200:
                ...
                logger.info("Initial state fetched")
        except Exception as e:
...
class ReferralBot:
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not firebase_url:
            await update.message.reply_text(
                ...
                f"Please send your Firebase URL:\n"
                f"`https://stockgro-xxxx.firebaseio.com/`",
                parse_mode='Markdown'
            )
    ...
    async def handle_firebase_url_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not firebase_url.startswith("https://") or "firebase" not in firebase_url:
            await update.message.reply_text(
                "❌ Invalid URL!\n"
                "Send like: `https://stockgro-xxxx.firebaseio.com/`",
                parse_mode='Markdown'
            )
            return WAITING_FOR_FIREBASE_URL
        ...
        await update.message.reply_text(
            ...
            f"Send /start to view dashboard."
        )
        return ConversationHandler.END
    ...
    async def show_progress(self, query, user_id: int):
        if not self.monitor:
            await query.edit_message_text("⚠️ Not initialized.")
            return
    ...
    async def show_success_list(self, query, user_id: int):
        ...
        if not events:
            await query.edit_message_text("📭 No successes yet.")
            return
    ...
    async def start_monitor(self, query, user_id: int):
        ...
        if not self.monitor:
            await query.edit_message_text("⚠️ Set Firebase URL first.")
            return
        ...
        await query.edit_message_text("✅ Monitoring started!", reply_markup=self._get_keyboard(user_id))
    ...
    async def stop_monitor(self, query, user_id: int):
        ...
        await query.edit_message_text("⏹ Monitoring stopped.", reply_markup=self._get_keyboard(user_id))
    ...
    async def set_target(self, query, user_id: int, target_str: str):
        ...
        if target_str == "custom":
            await query.edit_message_text("Use: /settarget <number>")
            return
        
        try:
            target = int(target_str)
            if target <= 0:
                await query.edit_message_text("❌ Must be positive.")
                return
            
            self.monitor.update_user_target(user_id, target)
            await query.edit_message_text(f"✅ Target updated to {target}!", reply_markup=self._get_keyboard(user_id))
        except ValueError:
            await query.edit_message_text("❌ Invalid number.")
    ...
    async def show_admin_panel(self, query, user_id: int):
        ...
        text = (
            ...
            f"🟢 Monitor: {'Active' if self.monitor.running else 'Stopped'}\n"
        )
    ...
    async def show_stats(self, query, user_id: int):
        ...
        text = (
            f"📊 Statistics\n"
            f"{'═' * 25}\n"
            f"✅ Successes: {global_success}\n"
            f"❌ Failed: {global_failed}\n"
            f"📈 Rate: {int((global_success/(global_success+global_failed))*100) if (global_success+global_failed) > 0 else 0}%\n\n"
            f"📌 Recent 5:\n"
        )
        ...
        try:
            if update and hasattr(update, 'effective_message'):
                await context.bot.send_message(
                    ...
                    text="⚠️ Error occurred. Try again."
                )
        except Exception:
            pass
...
def main():
    ...
    print(f"📡 Firebase: {'✅ Set' if FIREBASE_URL else '⚠️ User will set'}")
