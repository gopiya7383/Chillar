...
def _backup_name():
    return f"hosting_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"


def create_full_backup():
    """Create a ZIP containing hosting data and uploaded projects, excluding old backups/temp files."""
    os.makedirs(BACKUP_DIR, exist_ok=True)
    name = _backup_name()
    ...
    try:
        ...
        if os.path.isfile(RUNNING_SCRIPTS_DB):
            shutil.copy2(RUNNING_SCRIPTS_DB, os.path.join(data_root, 'running_scripts.json'))

        manifest = {
            'created_at': datetime.now().isoformat(),
            'format': 1,
            'contents': ['hosting_uploads', 'hosting_data/hosting_bot.db', 'hosting_data/running_scripts.json'],
            'note': 'BOT_TOKEN/ADMIN_ID are environment variables and are not stored in this backup.'
        }
        ...
        archive = shutil.make_archive(base, 'zip', temp_dir)
        return archive
    finally:
...
def backup_menu_markup():
    ...
    markup.row(
        types.InlineKeyboardButton(B('📤 𝐑𝐞𝐬𝐭𝐨𝐫𝐞 𝐁𝐚𝐜𝐤𝐮𝐩'), callback_data='backup_restore'),
...
def backup_callback(call):
    if not _admin_only(call.from_user.id):
        ...
        return
    bot.answer_callback_query(call.id)
    bot.edit_message_text(B('💾 𝐁𝐀𝐂𝐊𝐔𝐏 & 𝐑𝐄𝐒𝐓𝐎𝐑𝐄\n\n𝐒𝐚𝐟𝐞 𝐛𝐚𝐜𝐤𝐮𝐩 𝐨𝐟 𝐲𝐨𝐮𝐫 𝐡𝐨𝐬𝐭𝐢𝐧𝐠 𝐝𝐚𝐭𝐚, 𝐩𝐫𝐨𝐣𝐞𝐜𝐭𝐬 & 𝐦𝐞𝐭𝐚𝐝𝐚𝐭𝐚.\n🔐 𝐁𝐨𝐭 𝐭𝐨𝐤𝐞𝐧 𝐚𝐧𝐝 𝐀𝐃𝐌𝐈𝐍_𝐈𝐃 𝐚𝐫𝐞 𝐧𝐨𝐭 𝐬𝐭𝐨𝐫𝐞𝐝.'),
...
def backup_create_callback(call):
    ...
    bot.answer_callback_query(call.id, B('⏳ 𝐂𝐫𝐞𝐚𝐭𝐢𝐧𝐠 𝐛𝐚𝐜𝐤𝐮𝐩...'))
    msg = bot.send_message(call.message.chat.id, B('💾 𝐂𝐫𝐞𝐚𝐭𝐢𝐧𝐠 𝐛𝐚𝐜𝐤𝐮𝐩...'))
    try:
        path = create_full_backup()
        size_mb = os.path.getsize(path) / (1024 * 1024)
        with open(path, 'rb') as f:
            bot.send_document(call.message.chat.id, f, caption=B(f'✅ 𝐁𝐚𝐜𝐤𝐮𝐩 𝐂𝐫𝐞𝐚𝐭𝐞𝐝\n📦 𝐒𝐢𝐳𝐞: {size_mb:.2f} MB\n🔐 𝐒𝐞𝐜𝐫𝐞𝐭𝐬: 𝐄𝐱𝐜𝐥𝐮𝐝𝐞𝐝'))
        bot.edit_message_text(B('✅ 𝐁𝐚𝐜𝐤𝐮𝐩 𝐜𝐫𝐞𝐚𝐭𝐞𝐝 𝐬𝐮𝐜𝐜𝐞𝐬𝐬𝐟𝐮𝐥𝐥𝐲.'), call.message.chat.id, msg.message_id)
    except Exception as e:
        logger.exception('Backup creation failed')
        bot.edit_message_text(B(f'❌ 𝐁𝐚𝐜𝐤𝐮𝐩 𝐟𝐚𝐢𝐥𝐞𝐝: {str(e)[:500]}'), call.message.chat.id, msg.message_id)
...
def backup_restore_callback(call):
    if not _admin_only(call.from_user.id):
        bot.answer_callback_query(call.id, B('⚠️ 𝐀𝐝𝐦𝐢𝐧 𝐨𝐧𝐥𝐲'), show_alert=True); return
    pending_restore_users.add(call.from_user.id)
    bot.answer_callback_query(call.id)
    bot.send_message(call.message.chat.id, B('📤 𝐒𝐞𝐧𝐝 𝐭𝐡𝐞 𝐛𝐚𝐜𝐤𝐮𝐩 .𝐳𝐢𝐩 𝐟𝐢𝐥𝐞 𝐧𝐨𝐰.\n⚠️ 𝐑𝐞𝐬𝐭𝐨𝐫𝐞 𝐰𝐢𝐥𝐥 𝐫𝐞𝐩𝐥𝐚𝐜𝐞 𝐡𝐨𝐬𝐭𝐢𝐧𝐠 𝐝𝐚𝐭𝐚 𝐚𝐧𝐝 𝐩𝐫𝐨𝐣𝐞𝐜𝐭𝐬.'))


def restore_from_backup_bytes(file_content, file_name):
    """Restore project/data files from a backup ZIP with path traversal protection."""
    ...
    try:
        ...
        root = temp_dir
        uploads_src = os.path.join(root, 'hosting_uploads')
        data_src = os.path.join(root, 'hosting_data')
        if not os.path.isdir(uploads_src) and not os.path.isdir(data_src):
            raise ValueError('Invalid backup: hosting folders not found')

        # Create a safety backup before replacing anything.
        ...
        run_src = os.path.join(data_src, 'running_scripts.json')
        if os.path.isfile(run_src):
            # Old PIDs are stale after restore; force them to be rediscovered/restarted cleanly.
            with open(run_src, 'r', encoding='utf-8') as f:
                data = json.load(f)
            for item in data.get('running_scripts', []):
                item['process_pid'] = 0
...
def handle_file_upload(message):
    ...
    # Restore mode takes priority over normal hosting uploads.
    if user_id in pending_restore_users:
        ...
        try:
            ...
            bot.reply_to(message, B(f'✅ 𝐑𝐞𝐬𝐭𝐨𝐫𝐞 𝐜𝐨𝐦𝐩𝐥𝐞𝐭𝐞.\n🛡️ 𝐒𝐚𝐟𝐞𝐭𝐲 𝐛𝐚𝐜𝐤𝐮𝐩: {os.path.basename(safety)}\n🔄 𝐑𝐞𝐬𝐭𝐚𝐫𝐭 𝐭𝐡𝐞 𝐛𝐨𝐭 𝐭𝐨 𝐫𝐞𝐥𝐨𝐚𝐝 𝐫𝐞𝐬𝐭𝐨𝐫𝐞𝐝 𝐃𝐁 𝐝𝐚𝐭𝐚.'))
        except Exception as e:
