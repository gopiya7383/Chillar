...
def create_admin_panel():
    ...
    markup.row(
        types.InlineKeyboardButton(B('💾 𝐁𝐚𝐜𝐤𝐮𝐩 & 𝐑𝐞𝐬𝐭𝐨𝐫𝐞'), callback_data='backup'),
        types.InlineKeyboardButton(B('🖥️ 𝐀𝐝𝐦𝐢𝐧 𝐓𝐞𝐫𝐦𝐢𝐧𝐚𝐥'), callback_data='terminal')
    )
...
# ================================
# 💾 BACKUP & RESTORE + 🖥️ ADMIN TERMINAL
# ================================
BACKUP_DIR = os.path.join(HOSTING_DATA_DIR, 'backups')
os.makedirs(BACKUP_DIR, exist_ok=True)
pending_restore_users = set()
# user_id -> {'mode': 'file'|'all', 'user_id': int, 'file_name': str|None}
pending_backup_uploads = {}
# short callback token -> (target_user_id, file_name)
backup_target_tokens = {}
TERMINAL_TIMEOUT = 30
...
def _backup_name(prefix='hosting_backup'):
    return f"{prefix}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"


def _backup_target_token(target_user_id, file_name):
    """Create a short callback token for a project target."""
    raw = f"{target_user_id}:{file_name}:{time.time_ns()}"
    token = hashlib.sha256(raw.encode('utf-8')).hexdigest()[:12]
    backup_target_tokens[token] = (int(target_user_id), file_name)
    return token


def _backup_targets():
    """Return all uploaded projects as (user_id, file_name, file_type)."""
    targets = []
    for uid, files_list in sorted(user_files.items(), key=lambda x: str(x[0])):
        for item in files_list:
            try:
                file_name, file_type = item[0], item[1]
            except Exception:
                continue
            path = os.path.join(get_user_folder(uid), file_name)
            if os.path.isfile(path):
                targets.append((int(uid), file_name, file_type))
    return targets


def _project_backup_path(user_id, file_name):
    """Create a backup containing ONLY the selected project file."""
    user_folder = os.path.join(UPLOAD_BOTS_DIR, str(user_id))
    source = os.path.join(user_folder, sanitize_filename(file_name))
    if not os.path.isfile(source):
        raise FileNotFoundError(f"Project file not found: {file_name}")

    name = _backup_name(f"project_{sanitize_filename(file_name).replace('.', '_')}")
    final_path = os.path.join(BACKUP_DIR, name)
    temp_dir = tempfile.mkdtemp(prefix='project_backup_')
    try:
        project_dir = os.path.join(temp_dir, 'project')
        os.makedirs(project_dir, exist_ok=True)
        shutil.copy2(source, os.path.join(project_dir, os.path.basename(source)))
        manifest = {
            'backup_type': 'single_project',
            'created_at': datetime.now().isoformat(),
            'user_id': int(user_id),
            'file_name': os.path.basename(source),
            'file_type': os.path.splitext(source)[1].lstrip('.').lower(),
            'note': 'Only the selected project file is included. Secrets/tokens are not added separately.'
        }
        with open(os.path.join(temp_dir, 'BACKUP_INFO.json'), 'w', encoding='utf-8') as f:
            json.dump(manifest, f, indent=2)
        base = os.path.join(BACKUP_DIR, os.path.splitext(name)[0])
        return shutil.make_archive(base, 'zip', temp_dir)
    finally:
        shutil.rmtree(temp_dir, ignore_errors=True)


def create_full_backup():
    """Create an ALL BACKUP containing hosting data and all uploaded projects."""
    name = _backup_name()
    temp_dir = tempfile.mkdtemp(prefix='backup_')
    try:
        data_root = os.path.join(temp_dir, 'hosting_data')
        uploads_root = os.path.join(temp_dir, 'hosting_uploads')
        os.makedirs(data_root, exist_ok=True)
        os.makedirs(uploads_root, exist_ok=True)

        if os.path.isdir(UPLOAD_BOTS_DIR):
            shutil.copytree(UPLOAD_BOTS_DIR, uploads_root, dirs_exist_ok=True)
        if os.path.isfile(DATABASE_PATH):
            shutil.copy2(DATABASE_PATH, os.path.join(data_root, 'hosting_bot.db'))

        # Do NOT preserve live/stale PIDs in a backup. Recovery state is rebuilt safely.
        with open(os.path.join(data_root, 'running_scripts.json'), 'w', encoding='utf-8') as f:
            json.dump({'running_scripts': [], 'restored_at': datetime.now().isoformat()}, f, indent=2)

        manifest = {
            'backup_type': 'all',
            'created_at': datetime.now().isoformat(),
            'format': 2,
            'contents': ['hosting_uploads', 'hosting_data/hosting_bot.db'],
            'note': 'BOT_TOKEN/ADMIN_ID and live process PIDs are not stored in this backup.'
        }
        with open(os.path.join(temp_dir, 'BACKUP_INFO.json'), 'w', encoding='utf-8') as f:
            json.dump(manifest, f, indent=2)
...
def backup_menu_markup():
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(
        types.InlineKeyboardButton(B('➕ 𝐂𝐫𝐞𝐚𝐭𝐞 𝐁𝐚𝐜𝐤𝐮𝐩'), callback_data='backup_create'),
        types.InlineKeyboardButton(B('📤 𝐔𝐩𝐥𝐨𝐚𝐝 𝐁𝐚𝐜𝐤𝐮𝐩'), callback_data='backup_upload')
    )
    markup.row(
        types.InlineKeyboardButton(B('📋 𝐁𝐚𝐜𝐤𝐮𝐩 𝐇𝐢𝐬𝐭𝐨𝐫𝐲'), callback_data='backup_history'),
        types.InlineKeyboardButton(B('🔙 𝐁𝐚𝐜𝐤'), callback_data='admin_panel')
    )
    return markup


def _project_list_markup(action):
    """Show project names followed by an ALL BACKUP button."""
    markup = types.InlineKeyboardMarkup(row_width=1)
    backup_target_tokens.clear()
    targets = _backup_targets()
    for uid, file_name, file_type in targets:
        token = _backup_target_token(uid, file_name)
        icon = '🐍' if str(file_type).lower() == 'py' else '🟨'
        label = f'{icon} {file_name}'
        if len(str(user_files)) > 0 and len({t[0] for t in targets}) > 1:
            label += f'  •  {uid}'
        callback = f'backup_file_{token}' if action == 'create' else f'restore_file_{token}'
        markup.add(types.InlineKeyboardButton(B(label), callback_data=callback))

    if action == 'create':
        markup.add(types.InlineKeyboardButton(B('📦 𝐀𝐋𝐋 𝐁𝐀𝐂𝐊𝐔𝐏'), callback_data='backup_all'))
    else:
        markup.add(types.InlineKeyboardButton(B('📦 𝐀𝐋𝐋 𝐁𝐀𝐂𝐊𝐔𝐏'), callback_data='restore_all'))
    markup.add(types.InlineKeyboardButton(B('🔙 𝐁𝐚𝐜𝐤'), callback_data='backup'))
    return markup


def _show_project_list(call, action):
    if not _admin_only(call.from_user.id):
        bot.answer_callback_query(call.id, B('⚠️ 𝐀𝐝𝐦𝐢𝐧 𝐨𝐧𝐥𝐲'), show_alert=True)
        return
    bot.answer_callback_query(call.id)
    if action == 'create':
        title = '💾 𝐂𝐑𝐄𝐀𝐓𝐄 𝐁𝐀𝐂𝐊𝐔𝐏'
        desc = '📄 𝐉𝐢𝐬 𝐟𝐢𝐥𝐞 𝐤𝐚 𝐛𝐚𝐜𝐤𝐮𝐩 𝐜𝐡𝐚𝐡𝐢𝐲𝐞 𝐮𝐬 𝐟𝐢𝐥𝐞 𝐧𝐚𝐦𝐞 𝐩𝐚𝐫 𝐭𝐚𝐩 𝐤𝐚𝐫𝐨.\n📦 𝐀𝐋𝐋 𝐁𝐀𝐂𝐊𝐔𝐏 𝐬𝐞 𝐬𝐚𝐫𝐢 𝐩𝐫𝐨𝐣𝐞𝐜𝐭𝐬 𝐤𝐚 𝐛𝐚𝐜𝐤𝐮𝐩 𝐡𝐨𝐠𝐚.'
    else:
        title = '📤 𝐔𝐏𝐋𝐎𝐀𝐃 𝐁𝐀𝐂𝐊𝐔𝐏'
        desc = '📄 𝐉𝐢𝐬 𝐟𝐢𝐥𝐞 𝐤𝐨 𝐫𝐞𝐬𝐭𝐨𝐫𝐞 𝐤𝐚𝐫𝐧𝐚 𝐡𝐚𝐢 𝐮𝐬 𝐟𝐢𝐥𝐞 𝐧𝐚𝐦𝐞 𝐩𝐚𝐫 𝐭𝐚𝐩 𝐤𝐚𝐫𝐨.\n📦 𝐀𝐋𝐋 𝐁𝐀𝐂𝐊𝐔𝐏 𝐬𝐞 𝐩𝐨𝐨𝐫𝐚 𝐛𝐚𝐜𝐤𝐮𝐩 𝐫𝐞𝐬𝐭𝐨𝐫𝐞 𝐡𝐨𝐠𝐚.'
    text = B(f'{title}\n\n{desc}')
    try:
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=_project_list_markup(action))
    except Exception:
        bot.send_message(call.message.chat.id, text, reply_markup=_project_list_markup(action))


def backup_callback(call):
    if not _admin_only(call.from_user.id):
        bot.answer_callback_query(call.id, B('⚠️ 𝐀𝐝𝐦𝐢𝐧 𝐨𝐧𝐥𝐲'), show_alert=True); return
    bot.answer_callback_query(call.id)
    bot.edit_message_text(B('💾 𝐁𝐀𝐂𝐊𝐔𝐏 & 𝐑𝐄𝐒𝐓𝐎𝐑𝐄\n\n📄 𝐂𝐫𝐞𝐚𝐭𝐞 𝐁𝐚𝐜𝐤𝐮𝐩 → 𝐟𝐢𝐥𝐞 𝐬𝐞𝐥𝐞𝐜𝐭 𝐤𝐚𝐫𝐨.\n📦 𝐀𝐋𝐋 𝐁𝐀𝐂𝐊𝐔𝐏 → 𝐬𝐚𝐫𝐞 𝐩𝐫𝐨𝐣𝐞𝐜𝐭𝐬.\n📤 𝐔𝐩𝐥𝐨𝐚𝐝 𝐁𝐚𝐜𝐤𝐮𝐩 → 𝐬𝐚𝐦𝐞 𝐟𝐢𝐥𝐞 𝐤𝐚 𝐫𝐞𝐬𝐭𝐨𝐫𝐞.'),
                         call.message.chat.id, call.message.message_id, reply_markup=backup_menu_markup())


def backup_create_callback(call):
    _show_project_list(call, 'create')


def backup_upload_callback(call):
    _show_project_list(call, 'upload')


def _send_backup_file(chat_id, path, title):
    size_mb = os.path.getsize(path) / (1024 * 1024)
    with open(path, 'rb') as f:
        bot.send_document(chat_id, f, caption=B(f'✅ {title}\n📦 𝐒𝐢𝐳𝐞: {size_mb:.2f} MB'))


def backup_file_callback(call, token):
    if not _admin_only(call.from_user.id):
        bot.answer_callback_query(call.id, B('⚠️ 𝐀𝐝𝐦𝐢𝐧 𝐨𝐧𝐥𝐲'), show_alert=True); return
    target = backup_target_tokens.get(token)
    if not target:
        bot.answer_callback_query(call.id, B('❌ 𝐁𝐚𝐜𝐤𝐮𝐩 𝐭𝐚𝐫𝐠𝐞 𝐞𝐱𝐩𝐢𝐫𝐞𝐝. 𝐎𝐩𝐞𝐧 𝐭𝐡𝐞 𝐦𝐞𝐧𝐮 𝐚𝐠𝐚𝐢.'), show_alert=True); return
    uid, file_name = target
    bot.answer_callback_query(call.id, B('⏳ 𝐂𝐫𝐞𝐚𝐭𝐢𝐧𝐠...'))
    try:
        path = _project_backup_path(uid, file_name)
        _send_backup_file(call.message.chat.id, path, f'𝐁𝐚𝐜𝐤𝐮𝐩 𝐂𝐫𝐞𝐚𝐭𝐞𝐝: {file_name}')
        bot.send_message(call.message.chat.id, B(f'✅ 𝐁𝐀𝐂𝐊𝐔𝐏 𝐂𝐑𝐄𝐀𝐓𝐄𝐃\n📄 𝐅𝐢𝐥𝐞: `{file_name}`'), parse_mode='Markdown')
    except Exception as e:
        logger.exception('Single project backup failed')
        bot.send_message(call.message.chat.id, B(f'❌ 𝐁𝐚𝐜𝐤𝐮𝐩 𝐟𝐚𝐢𝐥𝐞𝐝: {str(e)[:700]}'))


def backup_all_callback(call):
    if not _admin_only(call.from_user.id):
        bot.answer_callback_query(call.id, B('⚠️ 𝐀𝐝𝐦𝐢𝐧 𝐨𝐧𝐥𝐲'), show_alert=True); return
    bot.answer_callback_query(call.id, B('⏳ 𝐂𝐫𝐞𝐚𝐭𝐢𝐧𝐠 𝐀𝐋𝐋 𝐁𝐀𝐂𝐊𝐔𝐏...'))
    try:
        path = create_full_backup()
        _send_backup_file(call.message.chat.id, path, '𝐀𝐋𝐋 𝐁𝐀𝐂𝐊𝐔𝐏 𝐂𝐑𝐄𝐀𝐓𝐄𝐃')
        bot.send_message(call.message.chat.id, B('✅ 𝐀𝐋𝐋 𝐁𝐀𝐂𝐊𝐔𝐏 𝐂𝐑𝐄𝐀𝐓𝐄𝐃 𝐒𝐔𝐂𝐂𝐄𝐒𝐒𝐅𝐔𝐋𝐋𝐘.'))
    except Exception as e:
        logger.exception('All backup failed')
        bot.send_message(call.message.chat.id, B(f'❌ 𝐀𝐥𝐥 𝐛𝐚𝐜𝐤𝐮𝐩 𝐟𝐚𝐢𝐥𝐞𝐝: {str(e)[:700]}'))


def backup_history_callback(call):
    if not _admin_only(call.from_user.id):
        bot.answer_callback_query(call.id, B('⚠️ 𝐀𝐝𝐦𝐢𝐧 𝐨𝐧𝐥𝐲'), show_alert=True); return
    bot.answer_callback_query(call.id)
    files = []
    for name in sorted(os.listdir(BACKUP_DIR), reverse=True):
        if name.endswith('.zip'):
            ...
            files.append(f'• {name} — {os.path.getsize(path)/(1024*1024):.2f} MB')
    text = '📋 𝐁𝐀𝐂𝐊𝐔𝐏 𝐇𝐈𝐒𝐓𝐎𝐑𝐘\n\n' + ('\n'.join(files[:20]) if files else '📭 𝐍𝐨 𝐛𝐚𝐜𝐤𝐮𝐩𝐬 𝐟𝐨𝐮𝐧𝐝.')
    bot.edit_message_text(B(text), call.message.chat.id, call.message.message_id, reply_markup=backup_menu_markup())


def _request_backup_upload(call, mode, target=None):
    if not _admin_only(call.from_user.id):
        bot.answer_callback_query(call.id, B('⚠️ 𝐀𝐝𝐦𝐢𝐧 𝐨𝐧𝐥𝐲'), show_alert=True); return
    uid = call.from_user.id
    if mode == 'file':
        target_uid, file_name = target
        pending_backup_uploads[uid] = {'mode': 'file', 'user_id': target_uid, 'file_name': file_name}
        text = f'📤 𝐔𝐏𝐋𝐎𝐀𝐃 𝐁𝐀𝐂𝐊𝐔𝐏\n\n📄 𝐓𝐚𝐫𝐠𝐞: `{file_name}`\n\n𝐀𝐛 𝐢𝐬𝐢 𝐟𝐢𝐥𝐞 𝐤𝐚 𝐛𝐚𝐜𝐤𝐮𝐩 `.zip` 𝐦𝐞 𝐬𝐞𝐧𝐝 𝐤𝐚𝐫𝐨.'
    else:
        pending_backup_uploads[uid] = {'mode': 'all', 'user_id': None, 'file_name': None}
        text = '📤 𝐔𝐏𝐋𝐎𝐀𝐃 𝐀𝐋𝐋 𝐁𝐀𝐂𝐊𝐔𝐏\n\n𝐀𝐛 𝐀𝐋𝐋 𝐁𝐀𝐂𝐊𝐔𝐏 `.zip` 𝐟𝐢𝐥𝐞 𝐬𝐞𝐧𝐝 𝐤𝐚𝐫𝐨.'
    bot.answer_callback_query(call.id)
    bot.send_message(call.message.chat.id, B(text), parse_mode='Markdown')


def restore_file_callback(call, token):
    if not _admin_only(call.from_user.id):
        bot.answer_callback_query(call.id, B('⚠️ 𝐀𝐝𝐦𝐢𝐧 𝐨𝐧𝐥𝐲'), show_alert=True); return
    target = backup_target_tokens.get(token)
    if not target:
        bot.answer_callback_query(call.id, B('❌ 𝐓𝐚𝐫𝐠𝐞 𝐞𝐱𝐩𝐢𝐫𝐞𝐝. 𝐎𝐩𝐞𝐧 𝐭𝐡𝐞 𝐦𝐞𝐧𝐮 𝐚𝐠𝐚𝐢.'), show_alert=True); return
    _request_backup_upload(call, 'file', target)


def restore_all_callback(call):
    _request_backup_upload(call, 'all')


def restore_from_backup_bytes(file_content, file_name):
    """Restore a full ALL backup safely. Live process PIDs are never restored."""
    ...
    try:
        ...
        uploads_src = os.path.join(temp_dir, 'hosting_uploads')
        data_src = os.path.join(temp_dir, 'hosting_data')
        if not os.path.isdir(uploads_src) and not os.path.isdir(data_src):
            raise ValueError('Invalid ALL backup: hosting folders not found')

        safety_path = create_full_backup()
        if os.path.isdir(uploads_src):
            os.makedirs(UPLOAD_BOTS_DIR, exist_ok=True)
            for item in os.listdir(uploads_src):
                src = os.path.join(uploads_src, item)
                dst = os.path.join(UPLOAD_BOTS_DIR, item)
                if os.path.isdir(src): shutil.copytree(src, dst, dirs_exist_ok=True)
                else: shutil.copy2(src, dst)

        db_src = os.path.join(data_src, 'hosting_bot.db')
        if os.path.isfile(db_src):
            shutil.copy2(db_src, DATABASE_PATH)

        # Always clear stale runtime recovery state after restore.
        with open(RUNNING_SCRIPTS_DB, 'w', encoding='utf-8') as f:
            json.dump({'running_scripts': [], 'restored_at': datetime.now().isoformat()}, f, indent=2)
        return safety_path
    finally:
        shutil.rmtree(temp_dir, ignore_errors=True)
...
def restore_single_project_from_bytes(file_content, target_user_id, target_file_name):
    """Restore ONLY one selected project file."""
    temp_dir = tempfile.mkdtemp(prefix='single_restore_')
    try:
        zip_path = os.path.join(temp_dir, 'backup.zip')
        with open(zip_path, 'wb') as f:
            f.write(file_content)
        ...
        project_dir = os.path.join(temp_dir, 'project')
        target_file_name = sanitize_filename(target_file_name)
        source = os.path.join(project_dir, target_file_name)
        if not os.path.isfile(source):
            # Accept the single-file backup's actual filename only when it matches the selected target.
            candidates = [os.path.join(project_dir, n) for n in os.listdir(project_dir)] if os.path.isdir(project_dir) else []
            candidates = [n for n in candidates if os.path.isfile(n)]
            if len(candidates) == 1 and os.path.basename(candidates[0]) == target_file_name:
                source = candidates[0]
            else:
                raise ValueError(f'Backup does not contain selected file: {target_file_name}')

        destination_dir = get_user_folder(target_user_id)
        destination = os.path.join(destination_dir, target_file_name)
        shutil.copy2(source, destination)
        file_type = os.path.splitext(target_file_name)[1].lstrip('.').lower() or 'py'
        save_user_file(target_user_id, target_file_name, file_type)
        recovery_system.remove_running_script(target_user_id, target_file_name)
        return destination
    finally:
        shutil.rmtree(temp_dir, ignore_errors=True)


def terminal_menu_markup():
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(
        types.InlineKeyboardButton(B('🖥️ 𝐑𝐮𝐧 𝐂𝐨𝐦𝐦𝐚𝐧𝐝'), callback_data='terminal_run'),
        types.InlineKeyboardButton(B('📊 𝐒𝐲𝐬𝐭𝐞𝐦 𝐈𝐧𝐟𝐨'), callback_data='terminal_info')
    )
    markup.add(types.InlineKeyboardButton(B('🔙 𝐁𝐚𝐜𝐤'), callback_data='admin_panel'))
    return markup


def terminal_callback(call):
    if not _admin_only(call.from_user.id):
        bot.answer_callback_query(call.id, B('⚠️ 𝐀𝐝𝐦𝐢𝐧 𝐨𝐧𝐥𝐲'), show_alert=True); return
    bot.answer_callback_query(call.id)
    bot.edit_message_text(B('🖥️ 𝐀𝐃𝐌𝐈𝐍 𝐓𝐄𝐑𝐌𝐈𝐍𝐀𝐋\n\n𝐎𝐧𝐥𝐲 𝐲𝐨𝐮𝐫 𝐀𝐃𝐌𝐈𝐍_𝐈𝐃 𝐜𝐚𝐧 𝐮𝐬𝐞 𝐭𝐡𝐢𝐬 𝐭𝐞𝐫𝐦𝐢𝐧𝐚𝐥.\n⏱️ 𝐂𝐨𝐦𝐦𝐚𝐧𝐝 𝐭𝐢𝐦𝐞𝐨𝐮𝐭: 30𝐬\n⚠️ 𝐑𝐮𝐧 𝐨𝐧𝐥𝐲 𝐜𝐨𝐦𝐦𝐚𝐧𝐝𝐬 𝐲𝐨𝐮 𝐭𝐫𝐮𝐬𝐭.'), call.message.chat.id, call.message.message_id, reply_markup=terminal_menu_markup())


def terminal_run_callback(call):
    if not _admin_only(call.from_user.id):
        bot.answer_callback_query(call.id, B('⚠️ 𝐀𝐝𝐦𝐢𝐧 𝐨𝐧𝐥𝐲'), show_alert=True); return
    bot.answer_callback_query(call.id)
    bot.send_message(call.message.chat.id, B('🖥️ 𝐒𝐞𝐧𝐝 𝐭𝐡𝐞 𝐬𝐡𝐞𝐥𝐥 𝐜𝐨𝐦𝐦𝐚𝐧𝐝 𝐭𝐨 𝐫𝐮𝐧:'))
    bot.register_next_step_handler(call.message, execute_terminal_command)
...
def execute_terminal_command(message):
    ...
    if not command:
        bot.reply_to(message, B('❌ 𝐄𝐦𝐩𝐭𝐲 𝐜𝐨𝐦𝐦𝐚𝐧𝐝.'))
        return
    try:
        proc = subprocess.run(command, shell=True, cwd=BASE_DIR, capture_output=True, text=True, timeout=TERMINAL_TIMEOUT)
        stdout = proc.stdout[-6000:] if proc.stdout else '(no stdout)'
        stderr = proc.stderr[-6000:] if proc.stderr else '(no stderr)'
        text = f'🖥️ 𝐓𝐄𝐑𝐌𝐈𝐍𝐀𝐋 𝐑𝐄𝐒𝐔𝐋𝐓\n\n📤 𝐎𝐮𝐭𝐩𝐮𝐭:\n{stdout}\n\n📥 𝐄𝐫𝐫𝐨𝐫:\n{stderr}\n\n🔢 𝐄𝐱𝐢𝐭 𝐂𝐨𝐝𝐞: {proc.returncode}'
        bot.reply_to(message, B(text))
    except subprocess.TimeoutExpired:
        bot.reply_to(message, B(f'⏱️ 𝐂𝐨𝐦𝐦𝐚𝐧𝐝 𝐭𝐢𝐦𝐞𝐝 𝐨𝐮𝐭 𝐚𝐟𝐭𝐞𝐫 {TERMINAL_TIMEOUT}𝐬.'))
    except Exception as e:
        bot.reply_to(message, B(f'❌ 𝐓𝐞𝐫𝐦𝐢𝐧𝐚𝐥 𝐞𝐫𝐫𝐨𝐫: {str(e)[:1000]}'))


def terminal_info_callback(call):
    if not _admin_only(call.from_user.id):
        bot.answer_callback_query(call.id, B('⚠️ 𝐀𝐝𝐦𝐢𝐧 𝐨𝐧𝐥𝐲'), show_alert=True); return
    try:
        ...
        text = (f'📊 𝐒𝐘𝐒𝐓𝐄𝐌 𝐈𝐍𝐅𝐎\n\n'
                f'🐍 𝐏𝐲𝐭𝐡𝐨𝐧: {sys.version.split()[0]}\n'
                f'🖥️ 𝐂𝐏𝐔: {psutil.cpu_percent(interval=0.2)}%\n'
                f'🧠 𝐑𝐀𝐌: {mem.percent}% ({mem.used/(1024**3):.2f}/{mem.total/(1024**3):.2f} GB)\n'
                f'💽 𝐃𝐢𝐬𝐤: {disk.percent}% ({disk.free/(1024**3):.2f} GB free)\n'
                f'📂 𝐏𝐫𝐨𝐣𝐞𝐜𝐭𝐬: {sum(len(v) for v in user_files.values())}\n'
                f'🟢 𝐑𝐮𝐧𝐧𝐢𝐧𝐠: {len(bot_scripts)}')
        bot.edit_message_text(B(text), call.message.chat.id, call.message.message_id, reply_markup=terminal_menu_markup())
        bot.answer_callback_query(call.id)
    except Exception as e:
        bot.answer_callback_query(call.id, B(f'❌ {str(e)[:150]}'), show_alert=True)
...
def handle_file_upload(message):
    ...
    # Backup upload/restore mode takes priority over normal hosting uploads.
    pending_backup = pending_backup_uploads.pop(user_id, None)
    if pending_backup and user_id in admin_ids:
        doc = message.document
        if not doc.file_name.lower().endswith('.zip'):
            bot.reply_to(message, B('❌ 𝐁𝐚𝐜𝐤𝐮𝐩 𝐫𝐞𝐬𝐭𝐨𝐫𝐞 𝐫𝐞𝐪𝐮𝐢𝐫𝐞𝐬 𝐚 .𝐳𝐢𝐩 𝐟𝐢𝐥𝐞.'))
            return
        try:
            info = bot.get_file(doc.file_id)
            content = bot.download_file(info.file_path)
            if pending_backup['mode'] == 'file':
                target_uid = pending_backup['user_id']
                target_name = pending_backup['file_name']
                restore_single_project_from_bytes(content, target_uid, target_name)
                bot.reply_to(message, B(f'✅ 𝐁𝐀𝐂𝐊𝐔𝐏 𝐑𝐄𝐒𝐓𝐎𝐑𝐄𝐃 𝐒𝐔𝐂𝐂𝐄𝐒𝐒𝐅𝐔𝐋𝐋𝐘\n📄 𝐅𝐢𝐥𝐞: `{target_name}`\n👤 𝐔𝐬𝐞𝐫: `{target_uid}`\n🔴 𝐒𝐭𝐚𝐭𝐮𝐬: 𝐒𝐭𝐨𝐩𝐩𝐞𝐝 (𝐬𝐚𝐟𝐞 𝐫𝐞𝐬𝐭𝐨𝐫𝐞)'), parse_mode='Markdown')
            else:
                safety = restore_from_backup_bytes(content, doc.file_name)
                bot.reply_to(message, B(f'✅ 𝐀𝐋𝐋 𝐁𝐀𝐂𝐊𝐔𝐏 𝐑𝐄𝐒𝐓𝐎𝐑𝐄𝐃 𝐒𝐔𝐂𝐂𝐄𝐒𝐒𝐅𝐔𝐋𝐋𝐘\n🛡️ 𝐒𝐚𝐟𝐞𝐭𝐲 𝐛𝐚𝐜𝐤𝐮𝐩: `{os.path.basename(safety)}`\n🔴 𝐑𝐮𝐧𝐧𝐢𝐧𝐠 𝐏𝐈𝐃𝐬: 𝐍𝐨𝐭 𝐫𝐞𝐬𝐭𝐨𝐫𝐞𝐝\n🔄 𝐑𝐞𝐬𝐭𝐚𝐫𝐭 𝐭𝐡𝐞 𝐛𝐨𝐭 𝐭𝐨 𝐫𝐞𝐥𝐨𝐚𝐝 𝐫𝐞𝐬𝐭𝐨𝐫𝐞𝐝 𝐃𝐁 𝐝𝐚𝐭𝐚.'), parse_mode='Markdown')
        except Exception as e:
            logger.exception('Backup restore failed')
            bot.reply_to(message, B(f'❌ 𝐁𝐚𝐜𝐤𝐮𝐩 𝐫𝐞𝐬𝐭𝐨𝐫𝐞 𝐟𝐚𝐢𝐥𝐞𝐝: {str(e)[:1000]}'))
        return

    # Legacy restore mode, if invoked by an older callback path.
    if user_id in pending_restore_users:
        pending_restore_users.discard(user_id)
        if user_id not in admin_ids:
            return
        doc = message.document
        if not doc.file_name.lower().endswith('.zip'):
            bot.reply_to(message, B('❌ 𝐑𝐞𝐬𝐭𝐨𝐫𝐞 𝐫𝐞𝐪𝐮𝐢𝐫𝐞𝐬 𝐚 .𝐳𝐢𝐩 𝐛𝐚𝐜𝐤𝐮𝐩.'))
            return
        try:
            info = bot.get_file(doc.file_id)
            content = bot.download_file(info.file_path)
            safety = restore_from_backup_bytes(content, doc.file_name)
            bot.reply_to(message, B(f'✅ 𝐁𝐀𝐂𝐊𝐔𝐏 𝐑𝐄𝐒𝐓𝐎𝐑𝐄𝐃\n🛡️ 𝐒𝐚𝐟𝐞𝐭𝐲 𝐛𝐚𝐜𝐤𝐮𝐩: {os.path.basename(safety)}'))
        except Exception as e:
            logger.exception('Restore failed')
            bot.reply_to(message, B(f'❌ 𝐑𝐞𝐬𝐭𝐨𝐫𝐞 𝐟𝐚𝐢𝐥𝐞𝐝: {str(e)[:1000]}'))
        return
...
BUTTON_HANDLERS = {
    ...
    B("💾 𝐁𝐚𝐜𝐤𝐮𝐩"): lambda m: bot.send_message(m.chat.id, B('💾 𝐁𝐀𝐂𝐊𝐔𝐏 & 𝐑𝐄𝐒𝐓𝐎𝐑𝐄'), reply_markup=backup_menu_markup()),
    B("🖥️ 𝐓𝐞𝐫𝐦𝐢𝐧𝐚𝐥"): lambda m: bot.send_message(m.chat.id, B('🖥️ 𝐀𝐃𝐌𝐈𝐍 𝐓𝐄𝐑𝐌𝐈𝐍𝐀𝐋'), reply_markup=terminal_menu_markup()),
}
...
def handle_callback_query(call):
    ...
    try:
        if data == 'upload':
            ...
            backup_create_callback(call)
        elif data == 'backup_upload':
            backup_upload_callback(call)
        elif data == 'backup_history':
            backup_history_callback(call)
        elif data == 'backup_all':
            backup_all_callback(call)
        elif data.startswith('backup_file_'):
            backup_file_callback(call, data[len('backup_file_'):])
        elif data.startswith('restore_file_'):
            restore_file_callback(call, data[len('restore_file_'):])
        elif data == 'restore_all':
            restore_all_callback(call)
        elif data == 'backup_restore':
            backup_upload_callback(call)
        elif data == 'terminal':
            terminal_callback(call)
        elif data == 'terminal_run':
            terminal_run_callback(call)
        elif data == 'terminal_info':
            terminal_info_callback(call)
...
if __name__ == '__main__':
    logger.info("="*50)
    logger.info("🚀 𝐇𝐎𝐒𝐓𝐈𝐍𝐆 𝐁𝐎𝐓 𝐕𝟑.𝟏 + 𝐁𝐀𝐂𝐊𝐔𝐏 + 𝐓𝐄𝐑𝐌𝐈𝐍𝐀𝐋")
