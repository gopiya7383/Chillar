...
class TelegramBot:
    ...
    def run(self):
        ...
        self.send_to_admin(
            "🤖 <b>StockGro Referral Bot Started!</b>\n\n"
...
def main():
    ...
    if not TELEGRAM_BOT_TOKEN or not TELEGRAM_ADMIN_ID or TELEGRAM_BOT_TOKEN == "YOUR_BOT_TOKEN_HERE":
        print("⚠️ Telegram bot not configured. Running in CLI mode...")
        print("   Set TELEGRAM_BOT_TOKEN and TELEGRAM_ADMIN_ID for Telegram control.")
        print()
...
def run_cli_mode():
    """Fallback CLI mode if Telegram not configured"""
    global target_count, firebase_url, referral_code, num_workers, success_count, failed_count, attempt_count, in_flight, stop_event
    
    print("Running in CLI mode...")
    print()
    
    panel_input = input("Enter Firebase panel link: ").strip()
    if not panel_input:
        print("Panel link cannot be empty.")
        sys.exit(1)
    
    firebase_url = parse_panel_link(panel_input)
    if not firebase_url:
        print("Could not parse Firebase panel link.")
        sys.exit(1)
    print(f"Firebase URL: {firebase_url}")
    print()
    
    refer_input = input("Enter Referral Code: ").strip()
    if not refer_input:
        print("Referral code cannot be empty.")
        sys.exit(1)
    
    referral_code = extract_referral_code(refer_input)
    if not referral_code:
        print("Could not parse referral code.")
        sys.exit(1)
    print(f"Referral Code: {referral_code}")
    print()
    
    count_str = input("How many successful refers? ").strip()
    if not count_str.isdigit() or int(count_str) <= 0:
        print("Invalid count.")
        sys.exit(1)
    target_count = int(count_str)
    
    workers_str = input("Parallel workers? (1-6): ").strip()
    if not workers_str.isdigit() or int(workers_str) < 1:
        num_workers = 1
    else:
        num_workers = min(int(workers_str), 8)
    
    print()
    print(f"Target: {target_count}, Workers: {num_workers}")
    print("Starting...")
    print("=" * 50)
    ...
    avail = device_pool.available_count()
    if avail == 0:
        print("No devices with phone numbers found.")
        sys.exit(1)
    print(f"Device pool ready: {avail} phone(s) available")
    print()
    ...
    try:
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            while not stop_event.is_set() and success_count < target_count:
                ...
                slots_available = min(num_workers - len(active_futures), remaining)
                
                if device_pool.available_count() == 0 and slots_available > 0:
                    device_pool.refresh()
                    if device_pool.available_count() == 0:
                        empty_pool_streak += 1
                        if empty_pool_streak >= 8 and not active_futures:
                ...
                # Show progress
                if success_count % 2 == 0 or success_count == target_count:
                    bar = render_progress_bar(success_count, target_count)
                    sys.stdout.write(f"\rProgress: {bar} | Success: {success_count}/{target_count} | Failed: {failed_count} | Pool: {device_pool.available_count()}")
                    sys.stdout.flush()
    ...
    print(f"Success: {success_count}/{target_count}")
